...
const Watch = () => {
  ...
  useEffect(() => {
    if (!animeDetails || !animeDetails.title_english) return;

    const fetchEmbedId = async () => {
      // Construct animegg episode URL based on anime title and current episode
      const slugifiedTitle = slugify(animeDetails.title_english || animeDetails.title);
      const episodeUrl = `https://www.animegg.org/${slugifiedTitle}-episode-${currentEpisode}`; //witch-watch-episode-1#dubbed

      const embedId = await fetchEmbedIdFromAnimegg(episodeUrl);
      setEmbedId(embedId);
      if (embedId) {
        setStreamUrlAnimeGG(`https://www.animegg.org/embed/${embedId}`);
      } else {
        setStreamUrlAnimeGG("");
      }
    };
            ...
            {focusMode && (
              <div className="modal-overlay" onClick={() => setFocusMode(false)}>
                <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                  {streamUrl ? (
                    <iframe
                      key={`focus-${server}-${currentEpisode}`}
