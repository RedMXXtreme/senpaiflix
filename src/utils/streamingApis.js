  ...
  /**
   * Get a random proxy URL with the provided URL encoded as a parameter
   * @param {string} url - The URL to be proxied
   * @returns {string} - A proxy URL with the encoded URL as parameter
   */
  ...
  */
  export async function fetchHindiDubEpisodeCount(animeName, episode) {
    try {
      ...
      const response = await fetchWithProxyRetry(animeUrl);
      const html = response.data;
