...
const jikanRecommendationsCache = new Map();
...
export async function fetchAnimeRecommendationsWithCache(animeId) {
  ...
  if (jikanRecommendationsCache.has(animeId)) {
    return jikanRecommendationsCache.get(animeId);
  }
  let retries = 3;
  let delay = 1000;
  while (retries > 0) {
    try {
      ...
      jikanRecommendationsCache.set(animeId, recommendations);
...
/**
 * Fetch embed ID from animegg episode page by scraping iframe src.
 * @param {string} episodeUrl - The URL of the animegg episode page.
 * @returns {Promise<string|null>} - The embed ID if found, otherwise null.
 */
export async function fetchEmbedIdFromAnimegg(episodeUrl) {
  try {
    ...
    // Use regex to find iframe src with /embed/{id}
    const iframeSrcMatch = html.match(/<iframe[^>]+src=["']([^"']*\/embed\/(\d+))["']/);
    if (iframeSrcMatch && iframeSrcMatch[2]) {
      return iframeSrcMatch[2]; // Return the embed ID
    }
    console.warn("Embed ID not found in animegg page.");
    return null;
  } catch (error) {
    console.error("Failed to fetch or parse animegg episode page:", error);
    return null;
  }
}
 ...
 */
const animeWorldIframeCache = new Map();

export async function fetchIframeUrlFromDesiDub(animeName, episode) {
  const cacheKey = `slugify(${animeName})-${episode}`;
  if (animeWorldIframeCache.has(cacheKey)) {
    console.log("Returning cached AnimeWorld iframe URL for:", cacheKey);
    return animeWorldIframeCache.get(cacheKey);
  }
  try {
    ...
    if (iframeSrcMatch && iframeSrcMatch[1]) {
      ...
      animeWorldIframeCache.set(cacheKey, iframeSrcMatch[1]);
