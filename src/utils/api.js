...
// New function to fetch IMDb id by anime name
export async function fetchImdbIdByAnimeName(animeName) {
  if (!animeName || typeof animeName !== 'string') {
    throw new Error('Invalid animeName parameter');
  }
  const url = `https://rest.imdbapi.dev/v2/search/titles?query=${encodeURIComponent(animeName)}`;
  try {
    ...
    // Find the best matching anime title from results
    // Here we assume the first result is the best match
    const bestMatch = data.results.find(item => item.titleType === 'tvSeries' || item.titleType === 'movie') || data.results[0];
    return bestMatch.id || null;
  } catch (error) {
    console.error('Error fetching IMDb id by anime name:', error);
    return null;
  }
}

// New function to fetch anime videos by anime id
export async function fetchAnimeVideos(animeId) {
  if (!animeId) {
    throw new Error('Invalid animeId parameter');
  }
  const url = `${JIKAN_API_BASE_URL}/anime/${animeId}/videos`;
  let attempt = 0;
  ...
  while (attempt <= MAX_RETRIES) {
    try {
      ...
      console.error('Error fetching anime videos from Jikan:', error);
      throw error;
    }
  }
}
